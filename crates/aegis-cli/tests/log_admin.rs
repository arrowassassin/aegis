//! `aegis log` filters, `redact`, `purge`, and `stop` over the real CLI binary.
#![cfg(unix)]
...
fn stdout_of(mut c: Command, db: &std::path::Path) -> (bool, String) {
    let out = c.env("AEGIS_DB", db).env("NO_COLOR", "1").output().unwrap();
    (
        out.status.success(),
...
fn redact_refuses_without_id_or_filter() {
    ...
    let db = tmp.path().join("e.db");
    seed(&db);
    let out = aegis().arg("redact").env("AEGIS_DB", &db).output().unwrap();
